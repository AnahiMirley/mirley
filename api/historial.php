...
<body>
    ...
    <script>
        ...
        function formatearFecha(fechaSql) {
            if (!fechaSql) return '';
            var s = String(fechaSql).replace('T', ' ');
            var partes = s.split(' ');
            var fechaPart = partes[0] || '';
            var horaPart = partes[1] ? partes[1].substring(0, 5) : '';
            var fecha = fechaPart.split('-');
            if (fecha.length < 3) return s;
            return fecha[2] + '/' + fecha[1] + '/' + fecha[0] + (horaPart ? ' ' + horaPart : '');
        }
