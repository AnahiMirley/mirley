...
<body>
    ...
    <script>
        ...
        var CEDULA_CONSUMIDOR_FINAL = <?= json_encode(CEDULA_CONSUMIDOR_FINAL) ?>;
        ...
        function cargarClientes(pagina) {
            ...
            fetch('clientes.php?accion=listar&q=' + encodeURIComponent(q) + '&pagina=' + paginaActual + '&incluir_inactivos=' + incluirInactivos)
                ...
                .then(function (data) {
                    if (!data || data.error) {
                        cuerpoTabla.innerHTML = '<tr><td colspan="6" class="text-center text-danger py-4">' + (data && data.error ? escaparHtml(data.error) : 'Respuesta inválida del servidor') + '</td></tr>';
                        return;
                    }
