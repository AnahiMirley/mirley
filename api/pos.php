...
if (is_array($rawUsuario)) {
    $usuario = $rawUsuario;
} else {
    $usuario = [
        'id' => null,
        'usuario' => (string)$rawUsuario,
        'nombre' => (string)$rawUsuario,
        'rol' => null
    ];
}
...
// ID real del cliente "Consumidor Final"
...
if ($accion === 'procesar_venta') {
    ...
    if (!is_array($input) || empty($input['productos']) || !is_array($input['productos'])) {
    ...
    // Valores monetarios (validar y forzar tipo)
    $totalVenta  = filter_var($input['total'] ?? null, FILTER_VALIDATE_FLOAT);
    $pagoVenta   = filter_var($input['pago'] ?? null, FILTER_VALIDATE_FLOAT);
    $cambioVenta = filter_var($input['cambio'] ?? null, FILTER_VALIDATE_FLOAT);

    if ($totalVenta === false || $pagoVenta === false || $totalVenta === null || $pagoVenta === null) {
        http_response_code(400);
        echo json_encode(['error' => 'Los valores monetarios no son válidos']);
        exit();
    }

    // Validar estructura de productos y tipos
    $productos = [];
    foreach ($input['productos'] as $idx => $p) {
        $pid = isset($p['id']) ? (int)$p['id'] : 0;
        $pcant = isset($p['cantidad']) ? (int)$p['cantidad'] : 0;
        $pprecio = isset($p['precio']) ? filter_var($p['precio'], FILTER_VALIDATE_FLOAT) : false;

        if ($pid <= 0 || $pcant <= 0 || $pprecio === false) {
            http_response_code(400);
            echo json_encode(['error' => "Producto inválido en posición $idx"]);
            exit();
        }
        $productos[] = ['id' => $pid, 'cantidad' => $pcant, 'precio' => (float)$pprecio];
    }

    try {
        $pdo->beginTransaction();

        // Verificar stock para cada producto (bloqueo FOR UPDATE)
        ...
        foreach ($productos as $p) {
            ...
            $prodDb = $stmtCheckStock->fetch(PDO::FETCH_ASSOC);
        ...
        // Insertar venta (usar usuarioId normalizado)
...
<body>
    ...
    <script>
        ...
        document.getElementById('btnProcesar').addEventListener('click', function () {
            ...
            .catch(function (err) {
                mostrarAlerta(err.message || 'Error al procesar la venta', 'error');
            });
        });
